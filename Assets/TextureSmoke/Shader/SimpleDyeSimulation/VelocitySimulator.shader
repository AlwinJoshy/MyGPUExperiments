Shader "Unlit/VelocitySimulator"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed _FlowAmount, _Divergence, _Curl, _CurlRes;
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                ...
                // divergence values
                float2 dc = pc * pow(max(vc, 0), _Divergence);
                float2 df = pf * pow(max(vf, 0), _Divergence);
                float2 di = pi * pow(max(vi, 0), _Divergence);
                float2 dh = ph * pow(max(vh, 0), _Divergence);
                float2 dg = pg * pow(max(vg, 0), _Divergence);
                float2 dd = pd * pow(max(vd, 0), _Divergence);
                float2 da = pa * pow(max(va, 0), _Divergence);
                float2 db = pb * pow(max(vb, 0), _Divergence);
                float4 col = 0;
                col.xy = dc + df + di + dh + dg + dd + da + db + myBit.xy;
                col.xy *= 0.285 * 1.5 * _FlowAmount;
