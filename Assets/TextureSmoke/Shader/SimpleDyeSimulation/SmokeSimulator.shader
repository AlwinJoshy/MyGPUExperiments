Shader "Unlit/Simple/SmokeSimulator"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                ...
                float col = Luminance(tex2D(_MainTex, i.uv + shift + fixed2(_MainTex_TexelSize.x, _MainTex_TexelSize.y))) +
                Luminance(tex2D(_MainTex, i.uv + shift + fixed2(_MainTex_TexelSize.x, 0))) +
                Luminance(tex2D(_MainTex, i.uv + shift + fixed2(_MainTex_TexelSize.x, -_MainTex_TexelSize.y))) +
                Luminance(tex2D(_MainTex, i.uv + shift + fixed2(0, -_MainTex_TexelSize.y))) +
                Luminance(tex2D(_MainTex, i.uv + shift + fixed2(-_MainTex_TexelSize.x, -_MainTex_TexelSize.y))) +
                Luminance(tex2D(_MainTex, i.uv + shift + fixed2(-_MainTex_TexelSize.x, 0)))+
                Luminance(tex2D(_MainTex, i.uv + shift + fixed2(-_MainTex_TexelSize.x, _MainTex_TexelSize.y))) +
                Luminance(tex2D(_MainTex, i.uv + shift + fixed2(0, _MainTex_TexelSize.y)));
