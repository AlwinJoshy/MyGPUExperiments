...
Shader "PixelToWorldSpace/DepthShadow&GodRayOptimizedMiniGameFlat"
{
    Properties
    {
        ...
        Pass
        {
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                ...
                // z depth of point from camera
                    ...
                    if(SquareMagnitude(wLerpPos.xyz - _AttentionPoint.xyz) < pow(_FogRange,2 )){
                        tracedFogDencity += (isOverShade * _FogCtrl * strideFogdencity) + ((1 - isOverShade) * _GlowPower * strideFogdencity);
                        sigmaFog += ((isOverShade * _FogCtrl * strideFogdencity * _FogColor) + ((1 - isOverShade) * depthColorSample.xyz * _GlowPower * strideFogdencity));
                    }
                    if(tracedFogDencity > _MaxReachThrushold) break;
                }
                return fixed4(_FogColor.xyz, saturate(pow(tracedFogDencity, _Exponential)));
            }
            ENDCG
        }
    }
