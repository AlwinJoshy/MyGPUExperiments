Shader "MagicOrb/MagicOrb_Haze_BlueNoise"
{
    Properties
    {
        ...
        _HazeShift("Haze Shift", range(0,3)) = 1
    }
    SubShader
    {
        ...
        Pass
        {
            ...
            sampler2D _MainTex, _GlowTex, _SwaySample, _DitherPattern;
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                ...
                for(int n = 0; n < 120; n++)
                {
                    pos = i.worldPos - (i.camDir * _SampleStepDist * (n + 1 + ditherValue));
                    ...
                    fixed distanceFromSurface = max(sqrDist - _CentralSphereSize,0);
                    fixed2 swirlUV = GetUV(pos) + fixed2(_Time.x * 3, -_Time.x * 5);
                    fixed2 swirlShift = tex2D(_SwaySample, swirlUV.xy).xy * 2 - 1;
                    swirlShift += tex2D(_SwaySample, swirlUV.yx).xy * 2 - 1;
                    fixed3 glowTex = tex2D(_GlowTex, GetUV(pos) + swirlShift * distanceFromSurface * _HazeShift);
                    fixed lengthVar = SqrMag(glowTex)/3;
                    fixed distFromSurface = saturate(0.1 + (lengthVar * _ExtraLength) - max(sqrDist - _CentralSphereSize,0));
